...
use Dekode\FnuggWhetherPlugin\Common\Traits\Requester;
use Dekode\FnuggWhetherPlugin\Common\Utils\Errors;
use Dekode\FnuggWhetherPlugin\Config\Classes;
use Dekode\FnuggWhetherPlugin\Config\I18n;
use Dekode\FnuggWhetherPlugin\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function loadClasses( $classes ) {
		...
		foreach ( $classes as $class ) {
			if (
				isset( $class['on_request'] ) && is_array( $class['on_request'] )
			) {
				...
				isset( $class['on_request'] ) && ! $this->request( $class['on_request'] )
			) {
				continue;
			}
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'dekode_fnugg_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				do_action( 'dekode_fnugg_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'gutenberg-fnugg-whether-block' ),
					__FILE__,
					$err
				);
			}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		// [
		// 'title'   => __( 'Gutenberg Fnugg Whether Block classes are not being loaded by Composer\'s Autoloader' ),
