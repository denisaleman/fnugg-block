 ...
 *
 * @package   Dekode\FnuggWhetherPlugin\Common\Utils\Errors
...
namespace Dekode\FnuggWhetherPlugin\Common\Utils;
...
class Errors {
	...
	public static function wpDie( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' ) {
		if ( $message ) {
			...
			$title = $title ? $title : $plugin['name'] . ' ' . $plugin['version'] . ' ' . __( '&rsaquo; Fatal Error', 'gutenberg-fnugg-whether-block' );
			Errors::writeLog(
				[
			...
			$source = $source ? '<code>' .
				sprintf(  /* translators: %s: file path */
					__( 'Error source: %s', 'gutenberg-fnugg-whether-block' ), $source
				) . '</code><BR><BR>' : '';
	...
	public static function pluginDie( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' ) {
		if ( $message ) {
			$plugin = self::getPluginData();
			$title = $title ? $title : $plugin['name'] . ' ' . $plugin['version'] . ' ' . __( '&rsaquo; Plugin Disabled', 'gutenberg-fnugg-whether-block' );
			Errors::writeLog(
				[
					'title'     => $title . ' - ' . $subtitle,
					'message'   => $message,
			...
			global $dekode_fnugg_die_notice;
			$dekode_fnugg_die_notice = $error;
		...
		add_action( 'admin_init',
			static function () {
				deactivate_plugins( plugin_basename( DEKODE_FNUGG_PLUGIN_FILE ) ); // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
			}
		);
	}
